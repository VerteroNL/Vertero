...
export default function ContactPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError('Er ging iets mis. Probeer het opnieuw of mail naar support@vertero.nl.')
    } finally {
      setSending(false)
    }
  }
      ...
      <div className="max-w-3xl mx-auto px-6 py-16 pb-24">
        ...
        <div className="mb-12">
          ...
          <p className="text-white/40 text-base leading-relaxed max-w-md">
            Een vraag, opmerking of technisch probleem? Stuur een bericht en we reageren binnen één werkdag.
          </p>
        </div>

        <div className="bg-[#0d0d1c] border border-white/10 rounded-2xl p-7 sm:p-8">
          {sent ? (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <div>
                ...
                <textarea
                  ...
                  required
                  rows={5}
                  className="w-full bg-[#07070f] border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/20 outline-none focus:border-white/25 transition text-sm resize-none"
                />
              </div>
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Link href="/faq" className="bg-[#0d0d1c] border border-white/10 rounded-2xl p-5 hover:border-white/20 transition group">
            <p className="font-semibold text-sm mb-1">Veelgestelde vragen</p>
            <p className="text-white/35 text-xs">Misschien staat het antwoord al in de FAQ.</p>
          </Link>
          <a href="mailto:support@vertero.nl" className="bg-[#0d0d1c] border border-white/10 rounded-2xl p-5 hover:border-white/20 transition group">
            <p className="font-semibold text-sm mb-1">Liever direct mailen?</p>
            <p className="text-white/35 text-xs">support@vertero.nl</p>
          </a>
        </div>
